...
public class BusinessLogicImp implements IBusinessLogic {
    ...
    @Override
    public double calculateRevenue(YearMonth yearMonth) throws ReservationException {
        try {
    ...
    @Override
    public int calculateUnreservedCapacity(YearMonth yearMonth) throws ReservationException {
        try {
            LocalDate inputStart = yearMonth.atDay(1);
            LocalDate inputEnd = yearMonth.atEndOfMonth();
